...
/* ------------------------------------ */
if ( ! function_exists( 'typesolid_hex2rgb' ) ) {

	function typesolid_hex2rgb( $hex, $array=false ) {
		$hex = str_replace("#", "", $hex);
...
/*  Google fonts
/* ------------------------------------ */
if ( ! function_exists( 'typesolid_enqueue_google_fonts' ) ) {

	function typesolid_enqueue_google_fonts () {
...
add_action( 'wp_enqueue_scripts', 'typesolid_enqueue_google_fonts' ); 	


/*  Dynamic css output
/* ------------------------------------ */
if ( ! function_exists( 'typesolid_dynamic_css' ) ) {

	function typesolid_dynamic_css() {
		if ( get_theme_mod('dynamic-styles', 'on') == 'on' ) {
		
			// rgb values
			$color_1 = get_theme_mod('color-1');
			$color_1_rgb = typesolid_hex2rgb($color_1);
...
.themeform select, 
.themeform textarea { background: '.esc_attr( get_theme_mod('color-background') ).'!important; }
				'."\n";
			}
			// sticky bar color
			if ( get_theme_mod('color-sticky-bar','') != '' ) {
				$styles .= '
.entry-bar { background: '.esc_attr( get_theme_mod('color-sticky-bar') ).'!important; }
				'."\n";
			}
			...
			wp_add_inline_style( 'typesolid-style', $styles );	
		}
	}
	
}
add_action( 'wp_enqueue_scripts', 'typesolid_dynamic_css' );
